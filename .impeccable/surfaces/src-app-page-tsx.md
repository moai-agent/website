...
STORY: Why (names mean something), how (a worktree per task), drift (ahu notices change, never substitutes), honesty (says what it cannot see), when (more than one agent on one repo). Then install and GitHub.

FIRST VIEWPORT: Full-bleed moai. Centered `ahu` wordmark with RGB tear, one line beneath, GitHub link top right, scroll cue bottom.

FORM: One Session, dealt 1 of 3 (list rank 1), seed key 9a85c984.

Signature interaction: sticky terminal on desktop types each command as its margin note scrolls into view; moai dims with scroll.
