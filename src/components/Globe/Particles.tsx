...
export function Particles({ speed, fov, aperture, focus, curl, size = 512, ...props }: any) {
  ...
  const simRef:any = useRef<THREE.Mesh>(null)
  const renderRef:any = useRef<THREE.Mesh>(null)
  // Set up FBO
  const [scene] = useState(() => new THREE.Scene())
  const [camera] = useState(() => new THREE.OrthographicCamera(-1, 1, 1, -1, 1 / Math.pow(2, 53), 5))
